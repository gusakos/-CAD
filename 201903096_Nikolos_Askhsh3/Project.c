...
typedef struct {
    ...
    int num_dependent;      // keep count if the comp ouput is dependent on previous comp ouputs and is a critical path
    char* dependent[2];       // store every input dependent on previous components so you know its max 2
    int output;//
}component;
...
/** check the final output buffer for the outputs of the susystem and give them the correct values
...
int main(){
    ...
    subsystem* s= malloc(sizeof(subsystem));
     int a=getSubSystemFromFile("File_netlist.txt",s,l);
   // s->inputs= malloc(sizeof(char*)*3);
   a=readTestbench("testbench.txt",&s);
