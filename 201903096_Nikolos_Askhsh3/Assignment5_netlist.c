...
#define SUBTRACTOR "FULL_SUBTRACTOR"
...
needs the inputs name and id(gate) and every input  to built the adder
free must be done by the caller
if add-subb= 1 create one extra input for the subbader */
Full_adder* create_full_adder(char* input_A, char* input_B, char* input_C_in, char* input_Sub, char* name, int gate_number, int add_subb_control) {
    ...
    adder->inputs= malloc(sizeof(char*)*(adder->num_inputs+1));                         
    ...
    if(add_subb_control==1){
       // printf("%d",add_subb_control);
         adder->inputs[3] = strdup(input_Sub);
    }
    ...
    // free the temporary strings used
    free(output_u1);
    free(output_u2);
    free(input_u);
    return adder;
...
/* checks the lib of the entity given(FUL_ADDER or FULL_SUBTRACTOR) and creates the correct entity where every input and ouput is stored using a list of adders 
...
int getEntity(char* file, entity_adder* entity){
    ...
    /// set the last characters  into '\0' because they ere /n
    if(strcmp(entity->name,SUBTRACTOR)==0){
        temp[temp_len-2]='\0';
        temp[temp_len-1]='\0';
        temp[temp_len]='\0';
    } 
    ...
    //get the subb_add control signal
    if(strcmp(entity->lib,SUBTRACTOR)==0){
       num_read= read_line_from_file(&line,file,&len,offset);
           if(num_read==-1){
                printf("unable to read line ");
                exit(0);
       ...
       temp= split(&ptr," ");
       //printf("%s",temp);
       entity->inputSub= malloc(strlen(temp)+1);
       strncpy(entity->inputSub,temp,strlen(temp)+1);
    }
    else{
        entity->inputSub='\0';
    }
    ...
    if(strcmp(entity->lib,SUBTRACTOR)==0){
        entity->adders[0]=create_full_adder(entity->inputsA[entity->num_bits-1],entity->inputsB[entity->num_bits-1],entity->inputCin,entity->inputSub,entity->name,0,1);
    }
    else{

    entity->adders[0]=create_full_adder(entity->inputsA[entity->num_bits-1],entity->inputsB[entity->num_bits-1],entity->inputCin,entity->inputSub,entity->name,0,0);
    }
    ...
    for(int i=1; i<entity->num_bits; i++){
        ...
        if(strcmp(entity->lib,SUBTRACTOR)==0){
            entity->adders[i]= create_full_adder(entity->inputsA[entity->num_bits-(i+1)], entity->inputsB[entity->num_bits-(i+1)], entity->adders[i-1]->outputs[1],entity->inputSub,entity->name, i,1);
        }
        else{

            entity->adders[i]= create_full_adder(entity->inputsA[entity->num_bits-(i+1)], entity->inputsB[entity->num_bits-(i+1)], entity->adders[i-1]->outputs[1],entity->inputSub,entity->name, i,0);
        }
...
int createTable (entity_adder* entity,char** netlist_table){
    ...
    //Print Sub_adder control signal if thre is one
     if(strcmp(entity->lib,SUBTRACTOR)==0){

        offset+=snprintf(netlist_table[0]+offset,strlen("; ")+strlen(entity->inputSub),"%s;",entity->inputSub);
    ...
    // print Cout in first line
    offset+=snprintf(netlist_table[0]+offset,strlen(" \n")+strlen(entity->inputCin)+1,"%s\n",entity->outputCout);

    //print begin line
    snprintf(netlist_table[1],strlen("BEGIN %s NETLIST\n")+strlen(entity->name),"BEGIN %s NETLIST\n",entity->name);
    
    //print every adder compont with this form Ui ADDER Input[0] Input[1] Input[2]
    if(strcmp(entity->lib,SUBTRACTOR)==0){
        for (int i=0; i<entity->num_bits; i++){
        snprintf(netlist_table[i+2],50,"%s %s %s, %s, %s, %s \n",entity->adders[i]->id,entity->adders[i]->name,entity->adders[i]->inputs[0],entity->adders[i]->inputs[1],entity->adders[i]->inputs[2],entity->adders[i]->inputs[3]);    
    }
    }
    else{
        for (int i=0; i<entity->num_bits; i++){
...
int main(){
    ...
    b=getEntity("entity_subadder.txt",e);
    ...
    FILE *fp = fopen("netlistDon5.txt", "w");
    if (fp == NULL) {
