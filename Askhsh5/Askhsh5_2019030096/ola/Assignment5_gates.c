...
/**
 * A substem can be a single 1bit adder or subadder comprising of components aka gates
*/
typedef struct {
    char* ID;
    char* name;
...
/**
 * A netlist is the final struct comprising of every 1 bit adder
*/
typedef struct {
    char** inputs;
    char** outputs;
...
/*  gets a file cointaing a subsystem full_adder or subtractor made of components , each component is a gate. 
    if it is a subtractor one more input is readen
 ...
 int getSubSystemFromFile(char* file,char* def,subsystem* sub,library* l,char* id){
    ...
    // store the name 
    sub->name= malloc(strlen(temp)+1);
    strncpy(sub->name,temp,strlen(temp)+1);


    ptr= strstr(line,sub->name)+ strlen(sub->name) +1;
    if (ptr == NULL) {
            ...
            if(strcmp(temp_comp,l->gates[j]->name)==0){
                ...
                for(int p=0; p<l->gates[j]->num_inputs; p++){
                    ...
                    for(int n=0; n<(i+1); n++){
                        ...
                        printf("error at comp:%s and input:%s",sub->components[i]->ID,temp_comp);
                       // exit(0);
                    }
                    }
...
/* Reads the netlist file given by the user and check if it is a subbtractor or an adder
    creates full adders/subtractor with the  inputs given
    First 2 inputs of every adder  are given by the file and are inputs of the netlist too (Ai Bi) but 
    the last input is the carry_out of the last adder passed by its gate ID
    for the subtractor its ust 3 inputs given by the file
 ...
 int setNetlist(char* file, netlist* net, library* lib){
    ...
    char* ptr= strstr(line,substring)+ strlen(substring);
    if (ptr == NULL) {  
      printf(" inputs of netlist not found.\n");
      exit(-1);
    }


    //check if it is a full_adder or a sub_adder
    char * check= strstr(line,FULL_SUB_ADDER_COMP);

        if (check == NULL) {        
            check=strstr(line,FULL_ADDER_COMP);
            if(check == NULL){
                printf("Cant find netlist type neither full_adder Neithrer Subbadder\n");
                exit(-1);

            }
            else{
                net->type=FULL_ADDER_COMP;
            }
         
        }
        else{
            net->type=FULL_SUB_ADDER_COMP;
        }
    ...
    do{
        ...
        // check if its a full adder comp or a subadder comp and move the substring into the inputs
        if(strcmp(net->type,FULL_ADDER_COMP)==0){
            ...
            ptr= strstr(line,FULL_SUB_ADDER_COMP)+ strlen(FULL_SUB_ADDER_COMP)+1;
            if (ptr == NULL) {        
                printf("Full adder not found.\n");
                exit(-1);
            }
        ...
        //Check again what type of subysystem is given and read the correct subsystem from the library
        // first store the the frist full adder with hard coded ID=0 
        // we need to store hhim only once so if i=0 store it
        if(i==0){
            if(strcmp(net->type,FULL_SUB_ADDER_COMP)==0){
                getSubSystemFromFile("Comp_full_sub_adder.txt",FULL_SUB_ADDER_COMP,net->adders[0],lib,id);
            }
            else{
            ...
            if(strcmp(net->type,FULL_SUB_ADDER_COMP)==0){
                getSubSystemFromFile("Comp_full_sub_adder.txt",FULL_SUB_ADDER_COMP,net->adders[i],lib,id);
            }
            else{
                getSubSystemFromFile("Comp_full_adder.txt",FULL_ADDER_COMP,net->adders[i],lib,id);
            }
            //give the correct id to each subadder
            for(int j=0; j<net->adders[i]->num_comps; j++){
                if(strcmp(net->type,FULL_SUB_ADDER_COMP)==0){
                    sprintf(buffer,"U%d",j+(6*i));
                    
                }
                //give the correct id to each adder
                else{
    ...
    // if it is a subtractor e need 4 inputs
    if(net->type== FULL_SUB_ADDER_COMP){

        strcpy(net->adders[i]->inputs[0],inps[counter-3]);
        strcpy(net->adders[i]->inputs[1],inps[counter-2]);
        strcpy(net->adders[i]->inputs[2],inps[counter-1]);
        strcpy(net->adders[i]->inputs[3],inps[counter]);

    }
    //if it is an adder e need 3 inputs
    else{
...
int main(){
    ...
    int c= setNetlist("netlist_SUB.txt",n,l);
